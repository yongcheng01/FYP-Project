...
<html>
    ...
    <body>
        ...
        if(isset($_POST['visitorID']) && $_POST['visitorID'] > 0){
            ...
            if ($vDate != $today) {
                echo '<script>alert("Update Status Failed. Date is not Today");</script>';
                
            }
            elseif($vDate == $today && $time > $currentTimestamp){
                echo '<script>alert("Update Status Failed. It’s not time to visit yet.");</script>';
            }
            elseif($status != "Pending"){
                echo '<script>alert("Update Status Failed. Visitor status is not pending.");</script>';
            }
            else {
                $query = "UPDATE visitor SET visitorStatus = 'Visited' WHERE visitorID = '$id[0]'";
                mysqli_query($conn, $query);
                
                echo'<script>alert("Update Visitor Status Successful")</script>';
