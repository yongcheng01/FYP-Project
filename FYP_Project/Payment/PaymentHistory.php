<html>
    ...
    <body class="sub_page">
        ...
        <div class="menu-container" id="menuContainer">
            ...
            <a href="PaymentHistory.php" class="menu-item active" id="item2">History</a>
        </div>
        ...
        <div class="page">
            ...
            <div class="row">
                ...
                <a href="PaymentHistory.php" class="refresh">
                    <i class="las la-sync reload-icon" style="font-size: 25px; margin-top: 5px;"></i>
                        ...
                        $(document).ready(function() {
                            $("#live-search").keyup(function() {
                                ...
                                if (input != "") {
                                    $.ajax({
                                        url: "PaymentHistoryLiveSearch.php",
                                        method: "POST",
                                        data: {input: input},
		...
		<table class="table table-hover table-bordered">
		    ...
		    <tbody>
                        <?php
                        $status ="Paid";
                        $total_rows = $conn->query("SELECT payment.*, receipt.*
                                                    FROM payment
                                                    INNER JOIN receipt ON payment.paymentID = receipt.paymentID
                                                    WHERE (payment.ownerID = '$userID' OR payment.tenantID = '$userID') AND payment.paymentStatus = '$status'  
                                                   ORDER BY receipt.receiptID")->num_rows;              
                        $page = isset($_GET['page']) && is_numeric($_GET['page']) ? $_GET['page'] : 1;
                        $num_results_on_page = 5;
                        if ($stmt = $conn->prepare('SELECT payment.*, receipt.*
                                                    FROM payment
                                                    INNER JOIN receipt ON payment.paymentID = receipt.paymentID
                                                    WHERE (payment.ownerID = ? OR payment.tenantID = ?) AND payment.paymentStatus = ?  
                                                    ORDER BY receipt.receiptID 
                                                    LIMIT ?,?')) {        
                            // Calculate the page to get the results we need from our table.
                            $calc_page = ($page - 1) * $num_results_on_page;
                            $stmt->bind_param('iissi', $userID, $userID, $status, $calc_page, $num_results_on_page);
                            $stmt->execute();
                            // Get the results...
                            ...
                            if (mysqli_num_rows($result) > 0) {
                                ...
                                while ($row = mysqli_fetch_assoc($result)) {
                                    ...
                                    $no++;
                                    ?>
                             <?php } } else{ echo "<tr><td colspan='8'>No Payment Record found</td></tr>"; } } ?>
                    </tbody>
                </table>
