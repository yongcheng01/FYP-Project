<html>
    ...
    <body>
        ...
        if (isset($_POST['forgot_password'])) {
            ...
            $emailCheckQuery = "Select * FROM admin Where adminEmail = '$email'";
            $emailCheckResult = mysqli_query($conn, $emailCheckQuery);

            if ($emailCheckResult) {
                if (mysqli_num_rows($emailCheckResult) > 0) {
                    $code = rand(999999, 111111);
                    $updateQuery = "UPDATE admin SET code = $code WHERE adminEmail = '$email'";
                    ...
                    if ($updateResult) {
                        ...
                        try {
                            $mail->send();
                            header("Location: AdminVerifyEmail.php?success=We've sent a verification code to your Email&$email");
                        } catch (Exception $e) {
                            $msg = "<div class='alert alert-danger'>Failed while sending code! Error: {$mail->ErrorInfo}</div>";
                        }
                    } else {
                        header("Location: AdminForgotPassword.php?error=Failed while inserting data into database!");
                    }
                } else {
                    header("Location: AdminForgotPassword.php?error=Invalid Email Address");
                }
            } else {
                header("Location: AdminForgotPassword.php?error=Failed while checking email from database!");
            }
        }
        ?>
        <div class="forgot">
            <div class="forgotPassword">
                ...
                <a href="AdminLogin.php" class="ca" >Back</a>
                </form>
            </div>
        </div>
    </body>
