<html>
    ...
    <body>
        ...
        $otp_query = "SELECT * FROM owner WHERE code = $otp";
        ...
        if (mysqli_num_rows($otp_result) > 0) {
            ...
            $update_query = "UPDATE owner SET accountStatus = '$update_status' , code = $update_code WHERE code = $fetch_code";
            $update_result = mysqli_query($conn, $update_query);

            if ($update_result) {
                header("Location: OwnerLogin.php?success=Your account has been created successfully");
            } else {
                header("Location: OwnerActive.php?error=Failed To Insering Data In Database!");
            }
        } else {
            header("Location: OwnerActive.php?error=You enter invalid verification code!");
        }
    }
        ?>
        <div class="Active">
