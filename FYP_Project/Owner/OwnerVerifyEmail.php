<html>
    ...
    <body>
        ...
        if (isset($_POST['verifyEmail'])) {     
                ...
                $verifyQuery = "SELECT * FROM owner WHERE code = '$OTPverify'";
                $runVerifyQuery = mysqli_query($conn, $verifyQuery);
                if ($runVerifyQuery) {
                    if (mysqli_num_rows($runVerifyQuery) > 0) {
                        $newQuery = "UPDATE owner SET code = 0";
                        $run = mysqli_query($conn, $newQuery);
                        header("location: OwnerResetNewPassword.php");
                }else {    
                        header("Location: OwnerVerifyEmail.php?error=Invalid Verification Code!");
                    }
                } else {
                    header("Location: OwnerVerifyEmail.php?error=Failed while checking email from database!");
                }     
            }
        ?>
        
        <div class="VerifyEmail">
            <h2 class="title">Owner Verify Email</h2>
