    ...
    <body>
        ...
        if (isset($_POST['changePassword'])) {
            ...
            if (strlen($_POST['password']) < 8) {
                header("Location: OwnerResetNewPassword.php?error=Use 8 or more characters with a mix of letters, numbers & symbols!");
            } else {
                // if password not matched so
                if ($_POST['password'] != $_POST['re_password']) {
                    header("Location: OwnerResetNewPassword.php?error=Password not matched!");
                } else {
                    $email = $_SESSION['email'];
                    $updatePassword = "UPDATE owner SET ownerPassword = '$password' WHERE ownerEmail = '$email'";
                    $updatePass = mysqli_query($conn, $updatePassword) or die("Query Failed");
                    session_destroy();
                     header("Location: OwnerLogin.php?success=Password Reset Successful!");
                    exit();
                }
            }
        }
        ...
        <div class="ResetPassword">
            <h2 class="title">Owner Reset Password</h2>
